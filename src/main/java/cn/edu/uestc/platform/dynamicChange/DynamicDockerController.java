...
public class DynamicDockerController {
	/**
	 * 1.生成所有的卫星节点
	 */
	public void createAllNodes() {
		...
		for (String node : nodes) {
			ServerFactory.createDockerServer(os, node, "192.168.5." + i++);
			try {
	...
	public void t0() throws Exception {
		...
		for (NewLink link : linksLR) {
			PortUtils.addPort(os, IPCreateFactory.searchData(link, IPCreateFactory.createTableLR())[0],
					IPCreateFactory.searchData(link, IPCreateFactory.createTableLR())[2]);
			TimeUnit.SECONDS.sleep(2);
			PortUtils.addPort(os, IPCreateFactory.searchData(link, IPCreateFactory.createTableLR())[1],
		...
		for (String[] link : IPCreateFactory.createTableUD()) {
			PortUtils.addPort(os, link[0], link[2]);
			TimeUnit.SECONDS.sleep(2);
			PortUtils.addPort(os, link[1], link[3]);
			TimeUnit.SECONDS.sleep(2);
		}
		// 3.GEO之间的链接
		...
		for (NewLink link : linksGEOToMeo) {
			//应该有一张GEO TO LEO的表
			PortUtils.addPort(os, IPCreateFactory.searchData(link, IPCreateFactory.createTableGEOToMEO())[0],
					IPCreateFactory.searchData(link, IPCreateFactory.createTableGEOToMEO())[2]);
			TimeUnit.SECONDS.sleep(2);
			PortUtils.addPort(os, IPCreateFactory.searchData(link, IPCreateFactory.createTableGEOToMEO())[1],
	...
	public void change(TextArea text) {
		...
		DynamicDockerTask task = new DynamicDockerTask(text);
