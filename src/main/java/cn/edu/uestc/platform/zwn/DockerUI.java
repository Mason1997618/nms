...
public class DockerUI extends JFrame {
...
//				new DynamicController().change(text);
				new DynamicDockerThread(text).start();
			}
		});
		p.add(dyBut);
		conPane.add("North", p);
...
class DynamicDockerThread extends Thread {
	private TextArea text;

	public DynamicDockerThread(TextArea text) {
		this.text = text;
	}
